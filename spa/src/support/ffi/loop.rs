...
type CSourceFunc = extern "C" fn(source: *mut CSource);
...
#[repr(C)]
struct CSource {
    // this should be *mut, but mutable dererefence via dyn Any (when used as CLoopImpl inner) does
    // not work
    loop_: *const CLoop,
    func: CSourceFunc,
    data: *mut c_void,
...
impl CLoopImpl {
    ...
    fn add_source(
        ...
        Self::result_from((funcs.add_source)(c_loop.iface.cb.data, c_source))
    }
    ...
    fn update_source(loop_: &mut LoopImpl, source: &Source) -> std::io::Result<i32> {
        ...
        Self::result_from((funcs.update_source)(
    ...
    fn remove_source(loop_: &mut LoopImpl, fd: RawFd) -> std::io::Result<i32> {
        ...
        Self::result_from((funcs.remove_source)(
            c_loop.iface.cb.data,
            &mut source_impl.c_source as *mut CSource,
        ))
    }
...
pub fn make_native(loop_: &LoopImpl) -> *mut CInterface {
...
pub fn free_native(c_loop: *mut CInterface) {
    unsafe {
